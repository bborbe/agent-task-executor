...
- feat: add a `type` discriminator to the Config CRD (`job` | `service`; absent means `job`) and exempt `service` from the `taskType`/`taskTypes` requirement — the executor only knew one shape, a short-lived `batch/v1` Job spawned per task and phase, so there was no way to declare a long-running identity agent that is addressed directly rather than routed by task. `ConfigSpec.Type` is a typed enum (`AgentTypeJob`/`AgentTypeService` plus `AvailableAgentTypes`, rejected by `Validate()` when unrecognised), never a bare string, and the schema constrains it to a two-value enum so a typo fails at admission instead of silently routing an identity agent down the Job path. The `taskType` requirement — enforced twice, by the CEL rule in `configSpecValidations()` and by `ConfigSpec.Validate()` — now applies to job agents only: a service agent is addressed directly and carries neither field, so without the exemption every `type: service` Config would have been rejected at admission. The CEL rule leads with `!has(self.type)` so a Config omitting the field still resolves to job and keeps the requirement. `type` is declared in `configSpecProperties()` **and** the Helm chart's `crds/config-crd.yaml`, because this connector overwrites the cluster CRD on every executor start and a field present only in the chart is pruned within seconds — the v0.5.0 `maxConcurrentJobs` failure, where a pruned integer read as 0 and the feature was inert in prod for weeks. Both copies are covered by the guard tests that exist for that defect class.

- feat: reconcile a long-running StatefulSet for a Config whose `spec.type` is `service`, alongside the per-task Jobs the executor already spawns — the binary only knew the Job shape, so there was no workload that outlives a single task for a person to hold a conversation with. `ServiceReconciler` builds the StatefulSet (one replica, the Config as its controller owner, its own `volumeClaimTemplates` datadir mounted at `VolumeMountPath` or pi's `~/.pi/agent/`), and `ServiceReconcileLoop` re-ensures it once a minute, mirroring the existing task reconcile floor. Two details are load-bearing rather than incidental: the StatefulSet sets `persistentVolumeClaimRetentionPolicy.whenDeleted: Delete`, because Kubernetes retains `volumeClaimTemplates` claims by default and the session PVC would otherwise outlive the CR; and the executor stamps `AGENT_TYPE` from `spec.type` into the container env, so a runner can tell the shapes apart without the type being hand-wired into every CR. `AgentConfiguration` and the resolver carry the type through. A job Config is ignored by the reconciler rather than trusted to the caller, and an unset type resolves to job — both pinned by specs. `SERVICE_STORAGE_CLASS` selects the storage class, empty meaning the cluster default.
